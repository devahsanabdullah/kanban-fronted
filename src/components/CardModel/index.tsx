            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                <div className="bg-[#2B2C37] px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  ...
                  <p className="mt-3 text-[#828fa3]">{cardData?.description}</p>
                  <div>
                    <h1 className="mt-3 text-white">
                      subtask of ({count?.length} of{" "}
                  ...
                  {cardData?.subtasks?.map((obj: any) => {
                    return (
                      <>
                        <div
                          className="ml-2 mt-3  flex cursor-pointer content-center items-center rounded-md bg-[#20212C] p-2 pl-2 hover:bg-[#635CF740]"
                          onClick={() => handleSubTask(obj)}
                        >
                          <input
                            type="checkbox"
                            checked={obj?.isComplete === true ? false : true}
                          />
                          <h1
                            className={
                              obj?.isComplete === true
                                ? "ml-2 text-white "
                                : "ml-2 text-white line-through"
                            }
                          >
                            {obj?.title}
                          </h1>
                  ...
                  <div className="mt-3 flex flex-col">
                    ...
                    <select name="status" className={`${styles}`}>
                      <option
